...
module.exports = function() {
  ...
  this.Then(/^no categories are selected$/, async () => {
    ...
    await driver.wait(
      until.elementLocated(by.css('.category__container')),
      10000
    );

    const categories = await driver.findElements(
      by.css('.category__container > .category__button.selected')
    );
  ...
  this.Then(
    /^the "([^"]*)" input shows "([^"]*)"$/,
    async (input_name, expected_value) => {
      await driver.wait(
        until.elementsLocated(
          by.xpath(`//input[@name='${input_name}'][@value='${expected_value}']`)
        ),
        10000
      );
    }
  ...
  this.Given(/^the search summary says "([^"]*)"$/, async text => {
    await driver.wait(
      until.elementsLocated(
        by.xpath(
          `//section[@class='search__criteria']//*[text()[contains(.,'${text}')]]`
        )
      ),
      10000
    );
