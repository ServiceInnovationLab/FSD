...
describe('url.js', () => {
  ...
  describe('filters', () => {
    ...
    it('responds with empty string when given falsey param', () => {
      expect(filters(false)).toBe('');
    });

    it('responds with category string when given a valid category', () => {
      expect(filters('dogs')).toBe('&filters={"LEVEL_1_CATEGORY":"dogs"}');
    });
  });
  ...
  describe('requestBuilder', () => {
    ...
    it("doesn't clarify keyword when it's null", () => {
      ...
      searchVarsClone.category = categories.none;
    ...
    it('filters by category', () => {
      searchVarsClone.keyword = keywords.valid;
      searchVarsClone.category = categories.valid;
      searchVarsClone.addressLatLng = addressLatLngs.none;

      expect(requestBuilder(searchVarsClone)).toBe(
        encodeURI(
          `${API_PATH}datastore_search?distinct=true&resource_id=${RESOURCE_ID}&fields=${STATICFIELDS}&q=${searchVarsClone.keyword}${filters(
