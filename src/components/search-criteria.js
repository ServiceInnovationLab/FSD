...
export default class SearchCriteria extends Component {
  render() {
    ...
    const { keyword, address, region, category, numOfResultsDisplayed, showLocation } = this.props;
    const searchCriteria = createSearchCriteria(keyword, address, region, category, showLocation);

    return (
      <section className="search__criteria">
        {searchCriteria ? (
          <p>
            {searchCriteria} {resultsDisplayed(numOfResultsDisplayed)}
          </p>
        ) : (
